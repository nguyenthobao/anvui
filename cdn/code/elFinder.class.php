...
class elFinder {
	...
	const ERROR_NGUYHIEM       		= 'errNguyHiem';
	...
	protected function rename($args) {
		...
		if (preg_match('/.php$/',$name)) {
			return array('error' => $this->error(self::ERROR_RENAME, $name.' là nguy hiểm. Xin đổi tên khác !'));
		}
	...
	protected function upload($args) {
		...
		foreach ($files['name'] as $i => $name) {
			...
			if (preg_match('/^[<?php]+/',file_get_contents($tmpname))) {
				$result['warning'] = $this->error(self::ERROR_NGUYHIEM, $name .' chứa nội dung nguy hiểm.');
				break;
			}
